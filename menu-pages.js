...
function studioVisual(title,catId){
  ...
  const x=c.getContext('2d'); const hc=hashColor(title); const kind=kindFor(catId,title);
  ...
  if(kind==='cocktail'){
    const hue=(hc.b*3)%360;
    const liquid=x.createLinearGradient(0,-50,0,75);
    ...
    x.fillStyle='rgba(255,255,255,.16)';x.fillRect(-45,-5,75,6);
  }else if(kind==='soft'){
    ...
    const liq=x.createLinearGradient(0,-55,0,70);liq.addColorStop(0,'rgba(225,105,62,.82)');liq.addColorStop(1,'rgba(120,18,30,.92)');x.fillStyle=liq;x.fillRect(-42,-30,84,102);
    x.strokeStyle='rgba(255,255,255,.7)';x.lineWidth=5;x.beginPath();x.moveTo(8,-100);x.lineTo(-5,45);x.stroke();
  }else{
    const wine=kind==='wine'||kind==='sparkling';
    const bottleW=wine?60:82, bodyH=wine?150:135, neckH=wine?70:48;
    ...
    x.fillStyle=glass;x.beginPath();
    x.moveTo(-bottleW*.27,-bodyH*.5-neckH);x.lineTo(bottleW*.27,-bodyH*.5-neckH);
    x.lineTo(bottleW*.3,-bodyH*.5-10);x.quadraticCurveTo(bottleW*.52,-bodyH*.42,bottleW*.52,-bodyH*.24);
    x.lineTo(bottleW*.52,bodyH*.5);x.quadraticCurveTo(bottleW*.52+2,bodyH*.58,bottleW*.43+4,bodyH*.6);
    x.lineTo(-bottleW*.43-4,bodyH*.6);x.quadraticCurveTo(-bottleW*.52-2,bodyH*.58,-bottleW*.52,bodyH*.5);
    x.lineTo(-bottleW*.52,-bodyH*.24);x.quadraticCurveTo(-bottleW*.52,-bodyH*.42,-bottleW*.3,-bodyH*.5-10);x.closePath();x.fill();
  ...
  x.fillStyle='#e6b77e';x.font='700 10px Manrope,Arial';x.letterSpacing='2px';x.fillText((catId||'BAR').replace(/-/g,' ').toUpperCase().slice(0,22),22,33);
  x.fillStyle='#fff7ef';x.font='600 24px Cormorant Garamond,Georgia';const lines=wrapLines(x,title,185,3);
  lines.forEach((line,i)=>x.fillText(line,22,75+i*30));
...
items.forEach((item,i)=>{
  ...
  const img=document.createElement('img'); img.className='item-photo'; img.alt=item.querySelector('h3')?.textContent?.trim()||'';
  img.decoding='async';
  const cat=item.closest('.cat'); const catId=cat?.id||'';
  if(isBar){
    img.loading='eager'; img.src=studioVisual(img.alt,catId);
  }else{
    img.loading=i<6?'eager':'lazy';
    img.src=`assets/menu/items/food-${String(i+1).padStart(3,'0')}.jpg?v=20260921-item-photo-v1`;
  }
